...
const VgCamera: FC = () => {
  ...
  const { virtualBgType, virtualBgImage } = useSelector(
  ...
  useEffect(() => {
    if (virtualBgType === "image") {
      ...
      const compositeFrame = async (
        ...
        if (!ctxImg) return;
        const ctxImageData = ctx.getImageData(
          0,
          0,
          canvas.width,
          canvas.height
  ...
  return (
    <div className={classes.wrapperStyle}>
      ...
      <canvas
        id="canvas"
        className={
          virtualBgType === "none" ? classes.disableStyle : classes.canvasStyle
        }
        style={
          virtualBgType === "image"
            ? { backgroundImage: `url(${virtualBgImage})` }
            : {}
        }
        width={640}
        height={480}
